...
public class LoginGUI extends PokemonGUI {
    ...
    public void initElements() {
        ...
        this.addElement(this.username = new InputElement<>(this, centerX - 60.0F, centerY - 20.0F, 120, "", (input) -> {
        }));
        this.addElement(new LabelElement<>(this, I18n.translateToLocal("gui.password.name"), centerX - 60.0F, centerY + 10.0F));
        this.addElement(this.password = new PasswordInputElement<>(this, "", centerX - 60.0F, centerY + 20.0F, 120, (input) -> {
            PokemonHandler.authenticate(this.username.getText(), this.password.getText());
            this.mc.displayGuiScreen(new LoggingInGUI());
        }));
        this.addElement(new ButtonElement<>(this, I18n.translateToLocal("gui.login.name"), this.width - 60.0F, this.height - 20.0F, 60, 20, (button) -> {
            PokemonHandler.authenticate(this.username.getText(), this.password.getText());
            this.mc.displayGuiScreen(new LoggingInGUI());
            return true;
        }));
        this.addElement(new ButtonElement<>(this, I18n.translateToLocal("gui.cancel.name"), 0.0F, this.height - 20.0F, 60, 20, (button) -> {
