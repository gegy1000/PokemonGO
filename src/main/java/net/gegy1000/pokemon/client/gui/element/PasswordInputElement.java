...
public class PasswordInputElement<T extends IElementGUI> extends Element<T> {
    ...
    public PasswordInputElement(T gui, String text, float posX, float posY, int width, boolean editable, Consumer<PasswordInputElement<T>> onEnter, Function<Integer, Boolean> allowKey) {
        super(gui, posX, posY, width, 12);
        this.text = text != null ? text : "";
        this.editable = editable;
        this.onEnter = onEnter;
        this.allowKey = allowKey;
        this.passwordFontRenderer = new PasswordFontRenderer();
    }
    ...
    public void render(float mouseX, float mouseY, float partialTicks) {
        ...
        String displayString = this.passwordFontRenderer.trimStringToWidth(this.text.substring(this.lineScrollOffset), this.getWidth());
        ...
        if (!displayString.isEmpty()) {
            ...
            line = this.passwordFontRenderer.drawString(s, x + 3, y + 1 + this.getHeight() / 2 - this.passwordFontRenderer.FONT_HEIGHT / 2, LLibrary.CONFIG.getTextColor(), false);
        }
        ...
        if (!displayString.isEmpty() && verticalCursor && cursor < displayString.length()) {
            this.passwordFontRenderer.drawString(displayString.substring(cursor), line + 1, y + 1 + this.getHeight() / 2 - this.passwordFontRenderer.FONT_HEIGHT / 2, LLibrary.CONFIG.getTextColor(), false);
        }
        ...
        if (cursorEnd != cursor) {
            float selectionWidth = x + this.passwordFontRenderer.getStringWidth(displayString.substring(0, cursorEnd));
            this.drawCursorVertical(lineX + (this.selectionEnd > this.cursorPosition ? 0 : 1), y, selectionWidth + (this.selectionEnd < this.cursorPosition ? 2 : 3), y + this.getHeight() / 2 - this.passwordFontRenderer.FONT_HEIGHT / 2 + 1 + this.passwordFontRenderer.FONT_HEIGHT);
        }
    }
    ...
    public boolean mouseClicked(float mouseX, float mouseY, int button) {
        ...
        if (this.selected && button == 0 && this.editable) {
            ...
            String displayString = this.passwordFontRenderer.trimStringToWidth(this.text.substring(this.lineScrollOffset), this.getWidth());
            this.setCursorPosition(this.passwordFontRenderer.trimStringToWidth(displayString, width).length() + this.lineScrollOffset);
        }
        return false;
    }
    ...
    public void setSelectionPos(int position) {
        ...
        String displayText = this.passwordFontRenderer.trimStringToWidth(this.text.substring(this.lineScrollOffset), this.getWidth());
        int offset = displayText.length() + this.lineScrollOffset;

        if (position == this.lineScrollOffset) {
            this.lineScrollOffset -= this.passwordFontRenderer.trimStringToWidth(this.text, this.getWidth(), true).length();
        }
